...
<body>
  ...
  <?php
    include('koneksi.php');
    $ID = $_GET['ID'];
    $query = mysqli_query($con, "SELECT * FROM major WHERE ID = '$ID'");
    $row = mysqli_fetch_array($query);
    ?>

  <div class="container">

  <h2 class="mt-4">Edit Major</h2>
    <div class="card shadow bg-body-tertiary rounded mt-4">
      <div class="card-body">
        <form action="updateDataMajor.php" method="post">
          <div class="row">
            <div class="col">
              <input class="form-control me-2" name="major" placeholder="Name" value="<?php echo $row['NAME'] ?>" required>
            </div>
            <div class="col">
              <?php
              include('koneksi.php');
              ...
              <select class="form-control me-2" name="id_faculty" required>
                <option value="<?php echo $row['FACULTY_ID'] ?>"><?php
                            include('koneksi.php');
                            $major_id = $row['FACULTY_ID'];
                            $query_faculty = "SELECT NAME FROM faculty WHERE ID='$major_id'";
                            $result_faculty = mysqli_query($con, $query_faculty);
                            $old_faculty = mysqli_fetch_array($result_faculty);
                            echo $old_faculty['NAME'];
                            ?></option>
                <?php
